

<!-- HELPUS_DOCS_CLOUD_INDEX_START -->

# Documentação HelpUS

## Estado atual

O runtime multi-IA foi implementado, testado e publicado no commit `cb917bec48a73d44dfceb2c038738cd429a32134`.

A fase atual é a implantação cloud no Railway.

## Documentos principais

- [Arquitetura de produção](ai/HELPUS_PRODUCTION_ARCHITECTURE.md)
- [Roadmap Estratégico & Arquitetura Multissoluções](HELPUSAI_STRATEGIC_ARCHITECTURE_ROADMAP.md)
- [Implantação Railway](ai/HELPUS_RAILWAY_MULTI_AI_DEPLOYMENT.md)
- [Runbook](ai/HELPUS_PRODUCTION_RUNBOOK.md)
- [Checklist](ai/HELPUS_PRODUCTION_CHECKLIST.md)
- [Plano de auditoria cloud](ai/HELPUS_CLOUD_INFRA_AUDIT_PLAN.md)
- [Integração runtime](ai/HELPUS_RUNTIME_MULTI_AI_INTEGRATION.md)
- [Auditoria documental](ai/HELPUS_DOCS_AUDIT_2026-07-20.md)
- [Rollout Obsidian](obsidian/HELPUSAI_PRODUCTION_ROLLOUT.md)

A produção será executada na Vercel e no Railway. O computador local será usado apenas para desenvolvimento, testes e manutenção.

<!-- HELPUS_DOCS_CLOUD_INDEX_END -->
